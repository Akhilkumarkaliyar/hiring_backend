...
app.post('/forgot' ,function(req, res){
    ...
    connection.query("select * from h_users where mobile = ?" ,[mobile] ,function(error, result , fields){
        if(!!error){
			...
			if(numRows > 0)
			{
				...
				connection.query('update h_users SET ? where mobile = ?',[otp,mobile],function (error, results, fields) {
					if(!!error){
						res.send({
...
//************forgot password**************//
//************Change password**************//
app.post('/verifychange' ,function(req, res){
	var otp= req.body.otp;
	var password= req.body.password;
	var mobile= req.body.mobile;
	var saltRounds = 10;
	var sql ="Select * from h_users where mobile='"+mobile+"' and otp='"+otp+"'";
        ...
        if(!!error){
			...
			if(numRows > 0)
			{
				bcrypt.hash(req.body.password, saltRounds, function (err,hash) {
					var sql ="update h_users set password='"+hash+"' where mobile='"+mobile+"' and otp='"+otp+"'";
					connection.query(sql,function (error, result, fields) {
						if(!!error){
							res.send({
								  "message":"Something Went wrong!",
...
app.post('/alldata' ,function(req, res){
	...
	var sql = "select * from h_users where id !=1 LIMIT "+pagecount+","+endcount+"";
	connection.query(sql,function(error, result , fields){
        if(!!error){
            res.send({
				  "message":"Something Went wrong!",
			...
			if(Numrows > 0){
				var sql ="SELECT count(*) as rows FROM  h_users where id !=1";
						...
						if(numRows > 0)
						{
							...
							connection.query("select * from h_users where id = ?" ,[id] ,function(error, results , fields){
								if(!!error){
									...
									app.mailer.send('email', {
										//to: 'akhil.kaliyar1992@gmail.com', // REQUIRED. This can be a comma delimited string just p a normal email to field. 
...
app.post('/login', function(req, res){
	...
	var password = req.body.password;
	connection.query('SELECT * FROM h_users WHERE mobile = ? ',[mobile], function (error, results, fields)
	{
		if (error) 
		{
			res.send({
		...
		}else
		{
			if(results.length >0)
			{
				...
				{
					connection.query('update h_users set token =? WHERE mobile = ?',[token,mobile], function (error, result, fields)
					{
						var sess = req.session;  //initialize session variable
						req.session.userId = results[0].id; //set user id
						req.session.mobile = results[0].mobile;//set user name
						ssn = req.session;
					});
					if(result == true){
						connection.query('SELECT * FROM h_users WHERE mobile = ?',[mobile], function (error, resu, fields)
						{
							if (error) 
							{
								res.send({
...
//********Edit Register************//
//////*************************Category***************/////////////////
app.post('/categorys' ,function(req, res){
	var totalrecord = req.body.totalrecord;
    var pagevalue= req.body.pagevalue;
	var pagecount = pagevalue *10;
	var endcount = totalrecord;
	var sql = "select * from h_category where is_deleted = '0' LIMIT "+pagecount+","+endcount+"";
	connection.query(sql,function(error, result , fields){
		if(!!error){
			res.send({
				  "message":"Something Went wrong!",
			...
			var Numrows =result.length;
			if(Numrows > 0){
				var sql ="SELECT count(*) as rows FROM  h_category where is_deleted = '0'";
				connection.query(sql,function(error, results , fields){
					if(!!error){
						res.send({
							  "message":"Something Went wrong!",
...
app.get('/category' ,function(req, res){
	var sql = "select * from h_category where is_deleted = '0'";
	connection.query(sql,function(error, result , fields){
		if(!!error){
			res.send({
				  "message":"Something Went wrong!",
...
app.post('/booking' ,function(req, res){
	var userid = req.body.userid;
	var sql ="SELECT b.*,u.user_name as ufname,a.token,a.user_name as afname,a.mobile as artistno FROM  h_booking as b JOIN h_users as u ON u.id= b.userid JOIN h_users as a ON a.id= b.artistid where b.artistid='"+userid+"' || b.userid='"+userid+"'";
	connection.query(sql,function(error, result , fields){
		if(!!error){
...
app.post('/payment' ,function(req, res){
	...
	var pagecount = pagevalue *10;
	var endcount = totalrecord;
	var sql ="SELECT p.*,u.user_name as ufname FROM  h_user_payment as p JOIN h_users as u ON u.id= p.user_id LIMIT "+pagecount+","+endcount+"";
	connection.query(sql,function(error, result , fields){
		if(!!error){
			...
			var Numrows =result.length;
			if(Numrows > 0){
				var sql ="SELECT count(*) as rows FROM  h_booking where id !=1";
				connection.query(sql,function(error, results , fields){
					if(!!error){
						res.send({
							  "message":"Something Went wrong!",
					...
					if(numRows > 0)
					{
						...
						if (error) {
							...
							if(numRows > 0)
							{
								...
								var serverKey = 'AAAAtiXwtKU:APA91bE3xO52qNJ4pTaiUR6oSZ2uabXk2JdkrYMO5EiXR0a77OG4ZWFvjv-337ycW6Br2EH4nQ0furVFzS69oIhHeQd2we2w4QnrIMf2fKq-smD12Xes7CCulvQpYivzP8pcOMajVOhS'; //put the generated private key path here    
								
								var fcm = new FCM(serverKey);
							 
								var message = { //this may vary according to the message type (single recipient, multicast, topic, et cetera)
									to: dtoken, 
									collapse_key: 'AIzaSyACBK91tIuRMZeN6UfRxjO7l7dyc7p8qZI',
									
									notification: {
										title: 'Title of your push notification artist', 
										body: 'You have a new booking request, kindly respond by going to "Manage Request" section on Profile Section.' 
									},
...
app.post('/postcatid' ,function(req, res){
	...
	var sql ="SELECT p.*,c.name as catname,u.user_name as username,u.image as img FROM  h_post as p JOIN h_users as u ON u.id= p.userid JOIN h_category as c ON p.category= c.id where p.category ='"+cat_id+"' and p.is_deleted=0";
    connection.query(sql,function(error, result , fields){
        if(!!error){
            console.log(error);
			res.send({
	...
	connection.query(sql,function(error, results , fields){
		if(!!error){
			...
			if(numRows > 0)
			{
				...
				var sql ="SELECT p.*,c.name as catname,u.user_name as username,u.image as userimage, ( select GROUP_CONCAT(hpf.filetype,'~',IFNULL(hpf.video,0),'~',IFNULL(hpf.image,0)) from h_post_file hpf  WHERE hpf.post_id=p.id) AS post_data FROM  h_post as p JOIN h_users as u ON u.id= p.userid JOIN h_category as c ON p.category= c.id where p.is_deleted=0  and p.userid ='"+user_id+"'";
				connection.query(sql,function(error, postdata , fields){
					if(!!error){
						...
						connection.query(sql,function(error, socialinfo , fields){
							if(!!error){
								res.send({
									  "status":'2',
									  "message":"Error in Query"
								...
								connection.query(sql,function(error, follow , fields){
									if(!!error){
										...
										for(i in postdata)
										{
											if(postdata[i].post_data)
											{
												...
												for(j in post)
												{
													...
													post_data.push({ "image": (postIn[2] !=0)? postIn[2]:null,"filetype": postIn[0],"video": (postIn[1] !=0)?postIn[1]:null });
										...
										var arr ={};
										arr = results[0];
										arr.postinfo= postdata;
										info['userinfo']=arr;
										info['post']=socialinfo;
...
app.get('/getaccess', function(req, res){ 
		...
		var para={
			"grant_type":'password',
			"client_id":'2f64c2e523c29a96d7de',
			"client_secret":'16a2e28baef5cd695cde05e64b723e1b49889d2d',
			"username":'scheppendsolutions@gmail.com',
			"password":'scheppend@12345',
			"scope":'manage_videos',
		}
	...
	connection.query(sql, function (error, result, fields) {
		...
		if (error) {
			...
			if(numRows > 0)
			{
				...
				var sql ="SELECT p.*,u.user_name,u.image, ( select GROUP_CONCAT(hpl.liked) from h_user_post_like hpl  WHERE hpl.post_id=p.id) AS postlike FROM  h_user_post as p JOIN h_users as u ON u.id= p.user_id where user_id in (?) ORDER BY id";
...
app.get('/gettopuser' ,function(req, res){
	...
	var sql ="select * from h_users where is_popular ='1'";
	connection.query(sql,function(error, result , fields){
        if(!!error){
				  res.send({
				  "status":'2',
...
app.post('/upcoming' ,function(req, res){
	...
	var pagecount = pagevalue *10;
	var endcount = totalrecord;
	var sql = "select * from h_upcomingevent  where is_deleted='0' LIMIT "+pagecount+","+endcount+"";
	connection.query(sql,function(error, result , fields){
		if(!!error){
			res.send({
				  "message":"Something Went wrong!",
			...
			var Numrows =result.length;
			if(Numrows > 0){
				var sql ="SELECT count(*) as rows FROM  h_upcomingevent where is_deleted='0'";
				connection.query(sql,function(error, results , fields){
					if(!!error){
						res.send({
							  "message":"Something Went wrong!",
...
app.post('/deleteupcoming', function(req, res){ 
		
		var id = req.body.id;
		connection.query("Update h_upcomingevent SET is_deleted='1' where id =?",[id], function (error, result, fields) {
		if (error) {
			res.send({
			  "status":'3',
			  "failed":"Something went wrong!!"
			...
			if(numRows > 0)
			{
				connection.query("select * from h_upcomingevent where is_deleted = ?",0,function(error, result , fields){
					if(!!error){
						res.send({
							  "message":"Something Went wrong!",
							  "status":'3',
						...
						dinfo['category']=result;
						res.send({
							  "message":"Upcoming event updated Sucessfully",
							  "status":'1',
							  "data":dinfo
							});
					} 
				...
				res.send({
				  "status":'2',
				  "message":"Upcoming event not deleted sucessfully"
				});
			}
...
app.post('/eventid' ,function(req, res){
	var id= req.body.id;
    connection.query("select * from h_upcomingevent where id = ?" ,[id] ,function(error, result , fields){
        if(!!error){
            console.log('error in query ');
			res.send({
				  "status":'2',
			...
			{
				var eventinfoid = {};
				eventinfoid['eventinfoid']=result;
				res.send({
				  "message":"Post Detail",
				  "status":'1',
				  "data":eventinfoid
...
app.post('/createevent',upload.single('image'),function(req, res, next){ 
	var blog={
		"title":req.body.title,
		"event_date":req.body.eventdate,
		"description":req.body.description,
	...
	var user ={
		...
		connection.query('INSERT INTO h_upcomingevent SET ?',blog, function (error, result, fields) {
		if (error) {
			res.send({
			  "status":'2',
			  "failed":"error ocurred"
...
app.post('/updateevent',upload.single('image'),function(req, res, next){ 
	console.log(req.file);
	if(req.file ==undefined){
		var blog={
			"title":req.body.title,
		...
		//console.log(slug);return;
		var id = req.body.id;
		connection.query('Update h_upcomingevent  SET ? where id =?',[blog,id], function (error, result, fields) {
		if (error) {
			res.send({
			  "status":'3',
			  "failed":"Something went wrong!!"
			...
			if(numRows > 0)
			{
				...
				res.send({
				  "status":'2',
				  "message":"Event not updated sucessfully"
				});
			}
...
app.post('/likeunlike' ,function(req, res){
	...
	var sql ="select * from h_user_post_like where user_id='"+user_id+"' and post_id='"+post_id+"'";
    connection.query(sql,function(error, result , fields){
        if(!!error){
            console.log('error in query ');
			res.send({
			...
			if(numRows > 0)
			{
				var likedval = result[0].liked;
				if(likedval==1){
					var newlvale =2;
					var message ="Post unliked Successfully!!";
				}else{
					var newlvale =1;
					var message ="Post liked Successfully!!";
				}
				var sql ="update h_user_post_like set liked='"+newlvale+"' where user_id='"+user_id+"' and post_id='"+post_id+"'";
				connection.query(sql,function(error, result , fields){
					if(!!error){
						console.log('error in query ');
						res.send({
				...
				connection.query('INSERT INTO h_user_post_like SET ?',liked, function (error, result, fields) 
				{
					if(!!error){
						console.log('error in query ');
						res.send({
...
app.post('/changeposttype',function(req, res, next){ 
	...
	var sql ="Update h_post  SET is_featured='"+featured+"' where id ='"+postid+"'";
	connection.query(sql, function (error, result, fields) {
		if (error) {
			res.send({
			  "status":'3',
				...
				if(numRows > 0)
				{
					...
					connection.query(sql,function(error, result , fields){
						if(!!error){
							...
							postinfo['postinfo']=result;
							res.send({
								  "message":"Post Type Change Successfully",
								  "status":'1',
								  "data":postinfo
								});
						} 
...
app.post('/changeservicetype',function(req, res, next){ 
	//console.log(req.body);return;
	var totalrecord = req.body.totalrecord;
    var pagevalue= req.body.pagevalue;
	var pagecount = pagevalue *10;
	...
	var sql ="Update h_users  SET service='"+service+"' where id ='"+userid+"'";
	connection.query(sql, function (error, resulted, fields) {
		if (error) {
			res.send({
			  "status":'3',
			  "failed":"Something went wrong!!"
